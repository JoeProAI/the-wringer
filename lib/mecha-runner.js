...
if rev:
    reviewer = {
        ...
        "citations": rev.get("citations") or [],
    }
notes = run_data.get("notes") or {}

# Honest evidence: unique source URLs actually cited by workers/reviewer, and the
# real execution result of the chosen answer. The UI only calls something
# "verified" when one of these is real.
_seen_urls = set()
all_citations = []
for _c in (run_data.get("candidates") or []):
    for _cit in (_c.get("citations") or []):
        _u = _cit.get("url")
        if _u and _u not in _seen_urls:
            _seen_urls.add(_u)
            all_citations.append({"url": _u, "title": _cit.get("title") or ""})
for _cit in ((run_data.get("reviewer") or {}).get("citations") or []):
    _u = _cit.get("url")
    if _u and _u not in _seen_urls:
        _seen_urls.add(_u)
        all_citations.append({"url": _u, "title": _cit.get("title") or ""})
...
report = {
    ...
    "rounds": notes.get("rounds"),
    "rounds_log": notes.get("rounds_log"),
    "stop_reason": notes.get("stop_reason"),
    "conf_bar": notes.get("conf_bar"),
...
task = ""
try:
    task = open(run_dir + "/task.txt", encoding="utf-8").read()[:16000]
except Exception:
    pass
# Feed GAMMA the COMPLETE deliverable + candidates (opus has a 200k context),
# so its verdict reflects the whole answer instead of a truncated head — a short
# cap here makes a complete SUCCESS look PARTIAL.
cand_lines = []
...
# Real evidence summary — sources actually cited + the actual execution result.
# GAMMA must ground its "verified" language in this, never invent it.
_ev = report.get("evidence") or {}
_ver = report.get("verification") or {}
_cites = report.get("citations") or []
_exec = _ev.get("execution", "none")
if _exec == "pass":
    exec_line = f"Code from the answer was ACTUALLY EXECUTED and PASSED ({_ver.get('kind')} via \`{_ver.get('command')}\`, exit {_ver.get('exit_code')})."
elif _exec == "fail":
    exec_line = f"Code from the answer was ACTUALLY EXECUTED and FAILED ({_ver.get('reason')}; exit {_ver.get('exit_code')})."
elif _exec == "inconclusive":
    exec_line = f"Code from the answer was executed but the result was inconclusive ({_ver.get('reason')})."
else:
    exec_line = "No code was executed (the answer contained no runnable code, or execution was off)."
if _cites:
    cite_line = f"{len(_cites)} external source(s) were retrieved and cited by the agents:\\n" + "\\n".join(f"- {c.get('title') or c.get('url')} ({c.get('url')})" for c in _cites[:12])
else:
    cite_line = "No external sources were retrieved — this was a reasoning-only run with no live fact-checking."
evidence_summary = f"EXECUTION EVIDENCE: {exec_line}\\nSOURCE EVIDENCE: {cite_line}"
...
- '## Evidence' — state plainly what real evidence backs this result, using ONLY the EVIDENCE block below. List the cited sources (with URLs) if any, and the actual code-execution result if any. CRITICAL HONESTY RULE: do NOT use the word 'verified' (or 'confirmed'/'proven') about a factual claim unless a source was actually cited for it, and do NOT call code 'working'/'verified' unless it actually executed and passed. If there are no sources and no execution, say explicitly: 'This result is reasoning-only — no external sources were checked and no code was executed.'
...
EVIDENCE (the only basis for any 'verified' language):
{evidence_summary}
...
{(report.get('report') or '')[:60000]}

REVIEWER VERDICT (winner={report.get('winner')}, confidence={report.get('confidence')}):
{(rev.get('text') or '')[:16000]}

CANDIDATES:
{chr(10).join(cand_lines)[:90000]}
"""
payload = {
    "model": model,
...
start = time.time()
# Premium deck knobs (env-overridable so quality/cost can be tuned without a redeploy).
# externalAccess=view is REQUIRED: without it the deck is workspace-private and the
# customer (not a workspace member) cannot open their own "View HD Presentation" link.
gamma_theme = os.environ.get("GAMMA_APP_THEME", "canaveral")
gamma_img_model = os.environ.get("GAMMA_APP_IMAGE_MODEL", "flux-2-pro")
try:
    gamma_cards = int(os.environ.get("GAMMA_APP_NUM_CARDS", "10"))
except ValueError:
    gamma_cards = 10
payload = json.dumps({
    ...
    "format": "presentation",
    "themeId": gamma_theme,
    "numCards": gamma_cards,
    "cardSplit": "auto",
    "cardOptions": {"dimensions": "16x9"},
    "exportAs": "pdf",
    "additionalInstructions": "Executive-grade technical report. Bold punchy headings, concise bullets. Lean into the dark high-tech theme. AI-generated imagery. From The Wringer — a multi-agent verification platform. Keep every number and name exact. Make it premium.",
    "imageOptions": {"source": "aiGenerated", "model": gamma_img_model, "style": "cinematic, dark, high-tech, neon, futuristic 3D render, volumetric lighting, sleek"},
...
try:
    ...
    # Poll for completion (max 4 min — HD image models can take longer)
    gamma_url = None
    export_url = None
    for _ in range(48):
        time.sleep(5)
