...
function buildPlainText(report, sessionEmail) {
  ...
  const presentationUrl = safeHttpsUrl(report.gamma_presentation_url);
  const exportUrl = safeHttpsUrl(report.gamma_export_url);
  if (presentationUrl) {
    lines.push(`View HD Presentation: ${presentationUrl}`);
  }
  if (exportUrl) {
    lines.push(`Download PDF: ${exportUrl}`);
  }
  if (presentationUrl || exportUrl) {
    lines.push(``);
  }

  if (!presentationUrl && report.gamma) {
...
<pre style="background:#1f1812;border:1px solid #3a332a;padding:18px;font-family:'IBM Plex Mono',monospace;font-size:13px;line-height:1.55;white-space:pre-wrap;word-break:break-word;color:#d9d0bc;max-height:600px;overflow:auto;">${escapeHtml(truncate(report.gamma, MAX_REPORT_LENGTH))}</pre>`
  ...
  if (presentationUrl) {
    linksHtml.push(
      `<a href="${escapeHtml(presentationUrl)}" style="display:inline-block;background:#c8351f;color:#ede6d6;padding:14px 24px;font-family:'Big Shoulders Display',Arial,sans-serif;font-weight:800;text-transform:uppercase;letter-spacing:2px;font-size:16px;text-decoration:none;margin-right:12px;margin-bottom:12px;">View HD Presentation</a>`
    );
  }
  if (exportUrl) {
    linksHtml.push(
      `<a href="${escapeHtml(exportUrl)}" style="display:inline-block;background:#c8351f;color:#ede6d6;padding:14px 24px;font-family:'Big Shoulders Display',Arial,sans-serif;font-weight:800;text-transform:uppercase;letter-spacing:2px;font-size:16px;text-decoration:none;margin-bottom:12px;">Download PDF</a>`
    );
  }

  const presentationNote =
    !presentationUrl && report.gamma
...
export async function sendMechaDeliverable({ report, sessionId, runId }, dependencies = {}) {
  ...
  const email = dependencies.sessionEmail ?? await getSessionEmail(sessionId);
  if (!email) {
  ...
  try {
    ...
    const deliveryId = String(runId || report.run_id || "unknown").slice(0, 200);
    await resend.emails.send(
      {
        ...
        headers: {
          "X-Wringer-Run-ID": deliveryId,
        },
      },
      { idempotencyKey: `wringer-${deliveryId}` }
    );
